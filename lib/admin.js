 ...
 */
  ...
  this.add = function(props, callback) {
    API.call('admin/add',
      props,
      function(err, res) {
        if (err)
          callback(error('Creating admin failed with error: ' + err), null);
        else {
  ...
  this.addContext = function(properties, callback) {
    API.call('admin/context/add',
      properties,
      function(err, res) {
        if (err)
          callback(error('Adding context failed with error: ' + err), null);
        else {
          callback(null, res.body.content);
        }
      });
  ...
  this.deleteContext = function(id, callback) {
    API.del('admin/context/remove',
      {id: id},
      function(err, res) {
        if (err)
          callback(error('Removing context failed with error: ' + err), null);
        else
          callback();
      });
  }
